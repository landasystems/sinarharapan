<?php
if (isset($_GET['v'])) {$this->setPageTitle('Lihat Truks | ID : '. $model->id);
$this->breadcrumbs=array(
	'Truks'=>array('index'),
	$model->id,
);
}else{$this->setPageTitle('Edit Truks | ID : '. $model->id);
$this->breadcrumbs=array(
	'Truks'=>array('index'),
...
if (isset($_GET['v'])) {
	...
	'items'=>array(
		array('label'=>'Tambah', 'icon'=>'icon-plus', 'url'=>Yii::app()->controller->createUrl('create'), 'linkOptions'=>array(),'visible' => landa()->checkAccess('truk', 'c')),
                ...
                array('label'=>'Edit', 'icon'=>'icon-edit', 'url'=>Yii::app()->controller->createUrl('update',array('id'=>$model->id)), 'linkOptions'=>array(),'visible' => landa()->checkAccess('truk', 'u')),
